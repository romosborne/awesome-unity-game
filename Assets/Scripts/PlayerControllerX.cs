...
public class PlayerControllerX : MonoBehaviour
{
  ...
  private void Update()
  {
    ...
    if (!moveLimit.CanMove && !itsTheIntro)
    {
      directionX = 0;
    }
    else
    {
      directionX = controls.Level.move.ReadValue<float>();
    }

    //Used to flip the character's sprite when she changes direction
    //Also tells us that we are currently pressing a direction button
    if (directionX != 0) transform.localScale = new Vector3(directionX > 0 ? 1 : -1, 1, 1);
  ...
  private void FixedUpdate()
  {
    ...
    if (useAcceleration)
    {
      runWithAcceleration(directionX != 0);
    }
    else
    {
      if (onGround)
  ...
  private void runWithAcceleration(bool accelerating)
  {
